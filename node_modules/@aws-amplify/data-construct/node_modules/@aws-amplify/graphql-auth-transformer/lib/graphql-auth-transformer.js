...
class AuthTransformer extends graphql_transformer_core_1.TransformerAuthBase {
    constructor() {
        ...
        this.addFieldResolverForDynamicAuth = (ctx, def, typeName, fieldName) => {
            ...
            if (resolver) {
                resolver.addToSlot('finish', undefined, graphql_transformer_core_1.MappingTemplate.s3MappingTemplateFromString(this.getVtlGenerator(ctx, def.name.value).generateFieldResolverForOwner(fieldName), `${typeName}.${fieldName}.{slotName}.{slotIndex}.res.vtl`));
            }
            else {
                ...
                resolver = ctx.resolvers.addResolver(typeName, fieldName, new graphql_transformer_core_1.TransformerResolver(typeName, fieldName, graphql_transformer_common_1.ResolverResourceIDs.ResolverResourceID(typeName, fieldName), graphql_transformer_core_1.MappingTemplate.s3MappingTemplateFromString('$util.toJson({"version":"2018-05-29","payload":{}})', `${typeName}.${fieldName}.req.vtl`), graphql_transformer_core_1.MappingTemplate.s3MappingTemplateFromString(this.getVtlGenerator(ctx, def.name.value).generateFieldResolverForOwner(fieldName), `${typeName}.${fieldName}.res.vtl`), ['init'], ['finish']));
        ...
        this.protectFieldResolver = (ctx, def, typeName, fieldName, roles) => {
            ...
            if (ctx.api.host.hasResolver(typeName, fieldName)) {
                ...
                if (existingResolver) {
                    const resolver = ctx.resolvers.getResolver(typeName, fieldName);
                    resolver.addToSlot('auth', graphql_transformer_core_1.MappingTemplate.s3MappingTemplateFromString(fieldAuthExpression, `${typeName}.${fieldName}.{slotName}.{slotIndex}.req.vtl`), graphql_transformer_core_1.MappingTemplate.s3MappingTemplateFromString(fieldResponse, `${typeName}.${fieldName}.{slotName}.{slotIndex}.res.vtl`));
                    resolver.setScope(scope);
                }
                else {
        ...
        this.addAutoGeneratedRelationalFields = (ctx, def, allowedFields, fields) => {
            const typeDefinitions = ctx.inputDocument.definitions.filter((it) => it.kind === 'ObjectTypeDefinition');
